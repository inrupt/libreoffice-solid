...
void SolidDetailsContainer::set_visible( bool bShow )
{
    m_pDialog->m_xFTHost->set_visible(bShow);
    m_pDialog->m_xHostBox->set_visible(bShow);
    m_pDialog->m_xEDRoot->set_visible(false); // Not used for Solid
    ...
    if (bShow)
    {
        if (!m_xPodUrl)
        {
            m_xPodUrl = m_pDialog->m_xEDHost; // Reuse host field for Pod URL
            m_xPodUrl->connect_changed(LINK(this, SolidDetailsContainer, PodUrlChangeHdl));
        }
        
        m_xPodUrl->set_placeholder_text("https://your-pod.solidcommunity.net/");
        
        // Set host label to "Pod URL"
...
{
    OUString sPodUrl = m_xPodUrl ? m_xPodUrl->get_text().trim() : OUString();
    
    if (!sPodUrl.isEmpty() && m_bAuthenticated)
    {
        // Convert HTTPS pod URL to solid:// scheme
        if (sPodUrl.startsWith("https://"))
        {
            OUString sSolidUrl = "solid://" + sPodUrl.copy(8); // Remove "https://"
...
bool SolidDetailsContainer::setUrl( const INetURLObject& rUrl )
{
    ...
    if (bSuccess)
    {
        ...
        {
            // Convert solid:// back to https:// for display
            OUString sPodUrl = "https://" + sUrl.copy(8); // Remove "solid://"
            if (m_xPodUrl)
                m_xPodUrl->set_text(sPodUrl);
            m_sPodUrl = sPodUrl;
        }
    }
...
{
    m_sWebId = rUsername; // Store WebID as username
}

void SolidDetailsContainer::setPassword( const OUString& /*rPassword*/ )
{
    // Solid uses OAuth2/OIDC, not password authentication
}

bool SolidDetailsContainer::verifyScheme( const OUString& rScheme )
{
    return rScheme == "solid://" || rScheme == "solids://";
}
...
void SolidDetailsContainer::authenticate()
{
    ...
    if (sPodUrl.isEmpty())
    {
        // Show error: Pod URL required
        return;
    }
    
    // TODO: Implement Solid-OIDC authentication flow
    // This would involve:
    // 1. Discovering the OIDC issuer from the pod
    // 2. Opening browser for authentication
    // 3. Handling the callback and token exchange
...
IMPL_LINK(SolidDetailsContainer, AuthenticateHdl, weld::Button&, /*rButton*/, void)
{
    authenticate();
}

IMPL_LINK(SolidDetailsContainer, PodUrlChangeHdl, weld::Entry&, /*rEntry*/, void)
{
    // Reset authentication status when URL changes
    m_bAuthenticated = false;
    notifyChange();
}
